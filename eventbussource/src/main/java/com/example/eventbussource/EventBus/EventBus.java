...
public class EventBus {
    ...
    //key : 事件class类型 value:该事件所有父类与接口class类型
    ...
    private final boolean eventInheritance; //是否支持事件继承
    ...
    private void subscribe(Object subscriber, SubscriberMethod subscriberMethod) {
        ...
        //1.填充subscriptionsByEventType集合，加入到事件订阅者集合
        ...
        //2.填充typesBySubscriber集合，将该订阅事件类型加入到相应订阅者集合中
        ...
        if (subscriberMethod.sticky) {
            if (eventInheritance) {
                ...
                for (Map.Entry<Class<?>, Object> entry : entries) {
                    //遍历所有sticky事件，若该事件A是sticky列表中某个事件B子类或接口，则只需要发送事件B即可。
