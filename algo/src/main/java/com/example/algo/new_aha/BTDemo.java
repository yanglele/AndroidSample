 ...
 * mirrorCopyRec 10.1 判断两个树是否互相镜像：isMirrorRec
...
//		System.out.println(isInTheTree(r1, r3));
        ...
        // TreeNode headNode = convertBST2DLL(r1);
        // while (headNode != null) {
        // System.out.print(headNode.val + " ");
        // if (headNode.left != null)
        // System.out.print(headNode.left.val + " ");
        // if (headNode.right != null)
        // System.out.print(headNode.right.val + " ");
        // System.out.println();
    ...
    public static void printTreeDfs(TreeNode node) {
    ...
    public static void printTreePathDfs(TreeNode node, String s) {
    ...
    public static int getDepthRec(TreeNode root) {
        if (root == null) return 0;
        int left = getDepthRec(root.left) + 1;
    ...
    public static void postorderTraversal(TreeNode root) {
        if (root == null) return;
        Stack<TreeNode> inNodes = new Stack<>();
        Stack<TreeNode> outTreeNodes = new Stack<>();
        inNodes.push(root);
        while (!inNodes.isEmpty()) {
            TreeNode tmp = inNodes.pop();
            outTreeNodes.add(tmp);
            if (tmp.left != null) inNodes.add(tmp.left);
            if (tmp.right != null) inNodes.add(tmp.right);
        }

        while (!outTreeNodes.isEmpty()) {
            System.out.print(outTreeNodes.pop().val + " ");
        }
    }
    ...
    public static void levelTraversalRec(TreeNode root) {
        ...
        levelTraversalRecSub(root, 0, list);
        System.out.println(list);
    }

    public static void levelTraversalRecSub(TreeNode root, int level, ArrayList<ArrayList<Integer>> ret) {
        if (root == null) return;

        if (level >= ret.size()) ret.add(new ArrayList<Integer>());

        ret.get(level).add(root.val);
        levelTraversalRecSub(root.left, level + 1, ret);
        levelTraversalRecSub(root.right, level + 1, ret);
    }
    ...
    public static boolean isSameRec1(TreeNode r1, TreeNode r2) {
        if (r1 == null && r2 == null) return true;
        else if (r1 == null || r2 == null) return false;
        else if (r1.val != r2.val) return false;
        return isSameRec1(r1.left, r2.left) && isSameRec1(r1.right, r2.right);
    }
    ...
    public static boolean isSame1(TreeNode r1, TreeNode r2) {
        if (r1 == null && r2 == null) return true;
        else if (r1 == null || r2 == null) return false;
        ...
        while (!stack1.isEmpty() && !stack2.isEmpty()) {
            ...
            if (tmp1.left != null) stack1.push(tmp1.left);
            if (tmp1.right != null) stack1.push(tmp1.right);
            if (tmp2.left != null) stack2.push(tmp2.left);
            if (tmp2.right != null) stack2.push(tmp2.right);
        }
    ...
    public static void micriCopyRec1(TreeNode root) {
        ...
        micriCopyRec1(root.left);
        micriCopyRec1(root.right);
    }

    // 2. 不能破坏原来的树，返回一个新的镜像树
    public static TreeNode microCopyRec(TreeNode root) {
        if (root == null) return null;
        TreeNode root2 = new TreeNode(root.val);
        root2.left = microCopyRec(root.right);
        root2.right = microCopyRec(root.left);
        return root2;
    }
    ...
    public static boolean isMirrorRec(TreeNode r1, TreeNode r2) {
        ...
        // boolean left=isMirrorRec(r1.left, r2.right);
        // boolean right=isMirrorRec(r1.right, r2.left);
        // return left&&right;
    ...
    public static TreeNode getLastCommonParentRec1(TreeNode root, TreeNode n1, TreeNode n2) {
        if (isInTheTree(root.left, n1)) {
            if (isInTheTree(root.right, n2)) {
                return root;
            } else {
                return getLastCommonParentRec1(root.left, n1, n2);
            }
        } else {
            if (isInTheTree(root.left, n2)) {
                return root;
            } else {
                return getLastCommonParentRec1(root.right, n1, n2);
            }
        }
    }

    public static TreeNode getLastCommonParentRec3(TreeNode root, TreeNode n1, TreeNode n2) {
        int length = getDepth(root);
        int[] path1 = new int[length];
        int[] path2 = new int[length];
        int level = 0;
        return null;
    }

    public static boolean isInTheTree(TreeNode root, TreeNode n1) {
        if (root == null) return false;
        if (root.val == n1.val) return true;
        boolean left = isInTheTree(root.left, n1);
        boolean right = isInTheTree(root.right, n1);
        System.out.println("left= " + left);
        System.out.println("right= " + right);
        return left || right;
        // return isInTheTree(root.left, n1) || isInTheTree(root.right, n1);
    }

    public static void getNodePath(TreeNode root, TreeNode n1) {
        int length = getDepth(root);
        int[] path = new int[length];

        // getNodePathSub(root, n1, path, 0);

        boolean[] a = {false};
        getNodePathSub1(root, n1, path, 0, a);
    ...
    public static void getNodePathSub1(TreeNode root, TreeNode n1, int[] path, int level, boolean[] exit) {
        if (root == null) return;
        if (exit[0] == true) return;
        path[level] = root.val;
        if (root.val == n1.val) {
            exit[0] = true;
            path[level + 1] = Integer.MAX_VALUE;
        }

        getNodePathSub1(root.left, n1, path, level + 1, exit);
        getNodePathSub1(root.right, n1, path, level + 1, exit);

    }

    public static void getNodePathSub(TreeNode root, TreeNode n1, int[] path, int level) {
        if (root == null) return;
        path[level] = root.val;
        if (root.val == n1.val) {
            for (int i : path)
        ...
        getNodePathSub(root.left, n1, path, level + 1);
        getNodePathSub(root.right, n1, path, level + 1);

    }

    public static void getLastCommonParentRecSub1(TreeNode root, TreeNode n1, TreeNode n2, Integer level, int[] path1, int[] path2) {
        if (root == null) return;
        path1[level] = root.val;
        if (root.val == n1.val && root.val == n2.val) return;
